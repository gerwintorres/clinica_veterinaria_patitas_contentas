<?php
    $pagina_actual = 'contactanos';
    include 'includes/templates/header.php';
?>
<h1 class="contenedor titulo-h1-pagina">Contáctanos</h1>
<main class="formulario-contacto contenedor">
    <div class="formulario-contacto-izq">
        <h4>Contacto Clínica Veterinaria Patitas Contentas</h4>
        <p>Siempre estaremos disponibles para brindarte un gran servicio.</p>
        <div>
            <ul>
                <li> 
                    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="26" height="26" transform="matrix(1 0 0 -1 0 26)" fill="url(#pattern0_33_529)"/>
                        <defs>
                        <pattern id="pattern0_33_529" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_33_529" transform="scale(0.0111111)"/>
                        </pattern>
                        <image id="image0_33_529" width="90" height="90" xlink:href="data:image/png;base64,"/>
                        </defs>
                    </svg>
                    +57 323 303 9679
                </li>
                <li>
                    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="26" height="26" fill="url(#pattern0_33_532)"/>
                        <defs>
                        <pattern id="pattern0_33_532" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_33_532" transform="scale(0.0111111)"/>
                        </pattern>
                        <image id="image0_33_532" width="90" height="90" xlink:href="data:image/png;base64,"/>
                        </defs>
                    </svg>
                    patitascontentas@gmail.com
                </li>
                <li>
                    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="26" height="26" fill="url(#pattern0_33_534)"/>
                        <defs>
                        <pattern id="pattern0_33_534" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0_33_534" transform="scale(0.0111111)"/>
                        </pattern>
                        <image id="image0_33_534" width="90" height="90" xlink:href="data:image/png;base64,"/>
                        </defs>
                    </svg>
                    Calle 19 #12-45, Barrio San Francisco, Pereira, Risaralda, Colombia
                </li>
            </ul>
        </div>
        <svg class="circulo" width="138" height="138" viewBox="0 0 138 138" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="69" cy="69" r="69" fill="#2DAEB3" fill-opacity="0.5"/>
        </svg>


    </div>
    <form action="">
        <div class="campos">
            <div>
                <label for="nombre">Nombre(s)</label>
                <input class="inputs" type="text" id="nombre" name="nombre" required>
            </div>
            <div>
                <label for="nombre">Apellidos</label>
                <input class="inputs" type="text" id="nombre" name="nombre" required>
            </div>
        </div>
        <div class="campos">
            <div>
                <label for="email">Email</label>
                <input class="inputs" type="email" id="email" name="email" required>
            </div>
            <div>
                <label for="numero">Número de contacto</label>
                <input class="inputs" type="numero" id="numero" name="numero" required>
            </div>
        </div>
        <div class="campo-mensaje">
            <label for="mensaje">Mensaje</label>
            <textarea id="mensaje" name="mensaje" rows="1" placeholder="Escribe tu mensaje..." required></textarea>
        </div>
        <input class="boton-formulario-azul" type="submit" value="Enviar Mensaje">
    </form>
</main>
<?php
    include 'includes/templates/footer.php';
?>